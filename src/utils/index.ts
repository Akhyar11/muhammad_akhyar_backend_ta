...
export function calculateKMSStatus(
  ageInMonths: number,
  weightInKg: number,
  gender: string
) {
...
export function calculateHeightStatus(
  ageInMonths: number,
  heightInCm: number,
  gender: string
) {
  ...
  // Select reference data based on gender
  const referenceData =
    gender.toLowerCase() === "male" ? heightReferenceBoy : heightReferenceGirl;
  ...
  if (exactMatchAge || closestAgeIndex === 0) {
    ...
    // Interpolate thresholds
    severelyStuntedThreshold = interpolateHeight(
      referenceData.severelyStunted[lowerIndex].height,
      referenceData.severelyStunted[upperIndex].height,
      ageRatio
    );

    stuntedThreshold = interpolateHeight(
      referenceData.stunted[lowerIndex].height,
      referenceData.stunted[upperIndex].height,
      ageRatio
    );

    tallThreshold = interpolateHeight(
      referenceData.tall[lowerIndex].height,
      referenceData.tall[upperIndex].height,
      ageRatio
    );
  }

  // Determine status based on height thresholds
  if (heightInCm < severelyStuntedThreshold) return "Severely Stunted";
  if (heightInCm < stuntedThreshold) return "Stunted";
  if (heightInCm > tallThreshold) return "Tall";
  return "Normal";
}
...
export function calculateWeightForHeightStatus(
  heightInCm: number,
  weightInKg: number,
  gender: string
) {
  ...
  // Select reference data based on gender
  const referenceData =
    gender.toLowerCase() === "male" ? weightForHeightBoy : weightForHeightGirl;
  ...
  if (closestHeightIndex === -1) return "Unknown";
  ...
  const exactMatchHeight =
    referenceData.data[closestHeightIndex].height === heightInCm;
  ...
  if (exactMatchHeight || closestHeightIndex === 0) {
    ...
    const heightRatio =
      (heightInCm - lowerHeight) / (upperHeight - lowerHeight);

    // Interpolate thresholds
    severelyWastedThreshold = interpolateWeight(
      referenceData.data[lowerIndex].severelyWasted,
      referenceData.data[upperIndex].severelyWasted,
      heightRatio
    );

    wastedThreshold = interpolateWeight(
      referenceData.data[lowerIndex].wasted,
      referenceData.data[upperIndex].wasted,
      heightRatio
    );

    overweightThreshold = interpolateWeight(
      referenceData.data[lowerIndex].overweight,
      referenceData.data[upperIndex].overweight,
      heightRatio
    );
  }

  // Determine status based on weight thresholds
  if (weightInKg < severelyWastedThreshold) return "Severely Wasted";
  if (weightInKg < wastedThreshold) return "Wasted";
  if (weightInKg > overweightThreshold) return "Overweight";
  return "Normal";
}

// Fungsi untuk menghitung status BMI-untuk-umur
export function calculateBMIForAgeStatus(
  ageInMonths: number,
  bmi: number,
  gender: string
) {
  ...
  let severelyUnderweightThreshold,
  ...
  if (exactMatchAge || closestAgeIndex === 0) {
    ...
    // Interpolate thresholds
    severelyUnderweightThreshold = interpolateWeight(
      genderData[lowerIndex].severelyUnderweight,
      genderData[upperIndex].severelyUnderweight,
      ageRatio
    );

    underweightThreshold = interpolateWeight(
      genderData[lowerIndex].underweight,
      genderData[upperIndex].underweight,
      ageRatio
    );

    overweightThreshold = interpolateWeight(
      genderData[lowerIndex].overweight,
      genderData[upperIndex].overweight,
      ageRatio
    );

    obeseThreshold = interpolateWeight(
      genderData[lowerIndex].obese,
      genderData[upperIndex].obese,
      ageRatio
    );
  }

  // Determine status based on BMI thresholds
  if (bmi < severelyUnderweightThreshold) return "Severely Underweight";
  if (bmi < underweightThreshold) return "Underweight";
  if (bmi > obeseThreshold) return "Obese";
  if (bmi > overweightThreshold) return "Overweight";
  return "Normal";
}
